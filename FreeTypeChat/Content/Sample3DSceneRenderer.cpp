...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	auto createCharCacheTask = m_CharCache.LoadCharCacheResources();



	// Create and upload cube geometry resources to the GPU.
	auto createAssetsTask = (createPipelineStateTask && createGridTask && createCharCacheTask).then([this]() {
		auto d3dDevice = m_deviceResources->GetD3DDevice();
  ...
  //      NAME_D3D12_OBJECT(m_constantBuffer);
		...
		m_deviceResources->WaitForGpu();	// also first initialization of char chache texture
...
bool Sample3DSceneRenderer::GetGlyph(UINT _CharCode, GlyphInTexture & _Glyph)
{
	return m_CharCache.GetGlyph(_CharCode, _Glyph);
}
